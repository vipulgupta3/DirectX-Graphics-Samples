...
void FindOptimalPartitions(in uint groupThreadId)
{
    uint treeletBitmask = groupThreadId + 1; // Start from 1
    while (treeletBitmask < NumTreeletSplitPermutations)
    {
        AABB aabb;
        ...
        treeletBitmask += NumThreadsInGroup;
    }

    AABB nodeAABB = AABBBuffer[nodeIndex];
    float rootAABBSurfaceArea = ComputeBoxSurfaceArea(nodeAABB);

    if (groupThreadId < FullTreeletSize)
    {
        optimalCost[BIT(groupThreadId)] = CalculateCost(AABBBuffer[treeletToReorder[groupThreadId]], rootAABBSurfaceArea);
    }
    ...
    for (uint subsetSize = 2; subsetSize <= FullTreeletSize; subsetSize++)
    {
        ...
        uint bitmaskIndex = groupThreadId;
        while (bitmaskIndex < numTreeletBitmasks)
        {
            uint treeletBitmask = GetBitPermutation(subsetSize, bitmaskIndex);
            ...
            bitmaskIndex += NumThreadsInGroup;
        }
